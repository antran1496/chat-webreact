...
rxio.login = function (token, uin, callback) {
  ...
  if (rxio.token) {
    let deviceid = '01w' + rxio.token.slice(0, 8) + uin.toString().slice(-6)
    const socket = io(socketURL, { transports: ['websocket'], query: `session=${rxio.token}&deviceid=${deviceid}` })
    rxio.socket = socket
    if (socket) {
...
const checkConnected = function (callback) {
  ...
  rxio.socket.on('login', () => {
    rxio.connected = true
    callback()
    addHandlers()
    rxio.init_connected = false
    // rxio.registerProfileToken()
  })
...
rxio.createGroupCall = function (group, callback) {
  if (rxio.connected && group.group_id && group.type) {
    rxio.socket.emit('create_groupcall', {
      group_id: group.group_id,
      type: group.type
    }, (data) => {
      callback(data)
...
rxio.joinGroupCall = function (options, callback) {
  if (rxio.connected && options.message_id && options.group_id) {
    rxio.socket.emit('join_groupcall', {
...
rxio.pingGroupCall = function (options, callback) {
  if (rxio.connected && options.message_id && options.group_id) {
    rxio.socket.emit('ping_groupcall', {
      group_id: options.group_id,
      message_id: options.message_id
    }, () => {
    })
...
rxio.leaveGroupCall = function (options, callback) {
  if (rxio.connected && options.message_id && options.group_id) {
    rxio.socket.emit('leave_groupcall', {
      group_id: options.group_id,
      message_id: options.message_id
    }, (data) => {
      callback(data)
...
rxio.inviteGroupCall = function (options, callback) {
  if (rxio.connected && options.message_id && options.group_id && options.sender_uin && options.invited_uin) {
    rxio.socket.emit('invite_groupcall', {
      group_id: options.group_id,
      message_id: options.message_id,
      sender_uin: options.sender_uin,
      invited_uin: options.invited_uin
    }, (data) => {
      callback(data)
    })
  }
...
rxio.checkGroup = function (group, callback) {
  if (rxio.connected && group.type && group.occupants_uins) {
      rxio.socket.emit('check_group', {
        'type':group.type,
        'occupants_uins':group.occupants_uins
      }, (data) => {
        callback(data)
