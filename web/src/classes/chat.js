/*global translate*/
const { rxaget, rxChangeSlug ,changeColorSVG} = global.rootRequire('classes/ulti')
...
export function getNumMemberOnl(group,userid){
    let status_list = group.status_list
    let countMemberOnl = 1
    if(status_list){
      for(let i = 0 ; i < status_list.length ;i++ ){
        if(status_list[i].uin !==  userid.toString() && status_list[i].online_status === 1){
          countMemberOnl++
        }
      }
    }
...
export function checkNameGroup (group, users, userid) { 
  ...
  if (group && group.isDraft && group.group_fullname !== '' ) {
    return (group.fullnamegroup) ? group.fullnamegroup : group.group_fullname || ''
  } else {
    ...
    if (!isNaN(objmsg.type)) {
      ...
      if (objmsg.status !== 4) {
        if (objmsg.deleted_uins && objmsg.deleted_uins.constructor === Array && objmsg.deleted_uins.length > 0 && objmsg.deleted_uins.indexOf(userid) !== -1) {
          message = getDeleteMessStr( objmsg, users, userid)
        } else {
          let mess =''
          switch (objmsg.type) {
            case 1:
              ...
              if(objmsg.group_type === 4){
                ...
                try {
                  ...
                  if (receiver_uin && receiver_uin.length > 0) {
                    ...
                    message = mess + checkNameUser(rxaget(users, objmsg.sender_uin, {}),userid)
                  }
                } catch(e3) {}
              }
            ...
            case 6:
              ...
              } else if (attachments && attachments.groupcall && attachments.groupcall.groupcall_id) {
                message = 'Group Call'
              }
              break
            case 7:
...
export function getDeleteMessStr(objmsg,users,userid){
  let user = ''
  if(userid.toString() === objmsg.sender_uin.toString()){
    user = translate('You')
  }else{
    user = checkNameUser(rxaget(users, objmsg.sender_uin.toString(), {})) 
  }
  let message = user + translate(' unsent a message')
  return message
}