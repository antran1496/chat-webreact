...
export function isLeaderCheck(eleId,group) {
  let isLeaderFlag = false
  try {
    eleId = eleId.toString()
    let owner_uin = group.owner_uin.toString()
    if (eleId === owner_uin) {
      isLeaderFlag = true //is a leader
    }
    else if (group.occupants_uins.indexOf(owner_uin) === -1
      && group.admin_uins
      && group.admin_uins.indexOf(eleId) >= 0) {
      isLeaderFlag = true
    }
  } catch (error) { }
  return isLeaderFlag
...
export function changeTheme(themeChoose) {
  try {
    ...
    document.getElementById('chat-root').style.setProperty("--theme-border-checkmark-color", themeChoose.theme_border_checkmark_color);
...
export function changeThemeColor(themeChoose) {
  if (rxconfig.theme === 'galo') {
    try {
      ...
      document.getElementById('chat-root').style.setProperty("--theme-background_media_mess_receiver", themeChoose.galo.theme_background_media_mess_receiver);
  ...
  if (rxconfig.theme === 'default') {
    try {
      ...
      document.getElementById('chat-root').style.setProperty("--theme-background_media_mess_receiver", themeChoose.default.theme_background_media_mess_receiver);
      ...
      document.getElementById('chat-root').style.setProperty("--theme-filter-color-icon-upload", themeChoose.default.theme_filter_color_icon_upload );
      document.getElementById('chat-root').style.setProperty("--theme_color_background_groupcall", themeChoose.default.theme_color_background_groupcall);
    } catch (e) { }
  }
...
export function changeColorSVG(url) {
  if (typeof window !== 'undefined') {
    const color = localStorage.getItem('netaThemeColor')
    if (color == '"blueColor"') {
      if (url.indexOf('/default/icons') !== -1) {
        return url.replace('/default/icons', '/default/icons_orange')
      } else if (url.indexOf('/default/static') !== -1) {
        return url.replace('/default/static', '/default/static_orange')
      } else {
        return url  
      } 
    } else {
...
export const rxconfig = (function () {
  ...
  // Meet Server
  config.meet_endpoint = secret.meet_endpoint
  ...
  config.get_username = config.base_api_neta + '/api/users/by_phone.json'
  // config.get_phone = config.base_api_neta + '/api/users/by_like_username.json'
  // config.get_username = config.base_api_neta + '/api/users/by_username.json'
  // config.get_username = config.base_api_neta + '/api/users/by_like_username.json'
