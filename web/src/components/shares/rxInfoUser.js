...
const { rxaget, stringToColour, rxChangeSlug ,changeColorSVG} = global.rootRequire('classes/ulti')
        ...
        <div className='avatarClick_rectangle_2_1'>
          <div className='avatarClick_title_place'>
            ...
            <div className='avatarClick_closeIcon' onClick={() => this.closePopupInfoUser()}>
              <img className='icon-basic-up filter_img_class' src={changeColorSVG('./images/default/icons/icon-basic-up.svg')} alt='' />
            </div>
          </div>
          ...
          {/*<div className='avatarClick_body'>
            <div className='avatarClick_phone_place'>
              <div className='avatarClick_info_act'>
                <img className='icon_messenger icon_talk filter_img_class' src={changeColorSVG('./images/default/icons/messenger.svg')} alt='' onClick={e => this.createNewChat(rxaget(userInfo, 'id', ''))}/>
                <img className='icon_call icon_talk filter_img_class' src={changeColorSVG('./images/default/icons/callright.svg')} alt='' onClick={(e) => this.onClickStartCall('voicecall', rxaget(userInfo, 'id', ''))}/>
                <img className='icon_videocall icon_talk filter_img_class' src={changeColorSVG('./images/default/icons/videocall.svg')} alt='' onClick={(e) => this.onClickStartCall('videocall', rxaget(userInfo, 'id', ''))}/>
              </div>
            </div>
          </div>
          ...
          <div className='ztab-more-info'>
            {(this.user && this.user.phone.indexOf('ipa\\') !== -1) && <div className='ztab-more-info_phone'>
              <span className='ztab-more-info_phone-txt'>{translate('Customer Id')}</span>
              <span className='ztab-more-info_phone_number'>{rxaget(userInfo, 'customerid', '') || ''}</span>
            </div>}
            {(this.user && this.user.phone.indexOf('ipa\\') === -1) && <div className='ztab-more-info_phone'>
              <span className='ztab-more-info_phone-txt'></span>
              <span className='ztab-more-info_phone_number'></span>
            </div>}
            {!checkBlock(this.state.userClick_group, this.state.userid)&&<div className='ztab-more-info_act '>            
              <img className='icon32 ztab-icon-act filter_img_class' alt='' src={changeColorSVG('./images/default/icons/messenger.svg')} onClick={(e) => this.createNewChat(rxaget(userInfo, 'id', ''))} />
              <img className='icon32 ztab-icon-act filter_img_class' alt='' src={changeColorSVG('./images/default/icons/callright.svg')} onClick={(e) => this.onClickStartCall('voicecall', rxaget(userInfo, 'id', ''))} />
              <img className='videocall-icon ztab-icon-act filter_img_class' alt='' src={changeColorSVG('./images/default/icons/videocall.svg')} onClick={(e) => this.onClickStartCall('videocall', rxaget(userInfo, 'id', ''))}/>
            </div>}
          </div>
