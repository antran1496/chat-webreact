...
const { rxaget, stringToColour, rxChangeSlug, subString ,changeColorSVG} = global.rootRequire('classes/ulti')
...
class RxGroupList extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (nextPopup.status) {
      ...
      let flagCreateGroup = (['create_channel','create_group', 'add_member'].indexOf(nextPopup.type_popup) !== -1) ? true : false
      let flagCreateGroupPopup = (['create_channel'].indexOf(nextPopup.type_popup) !== -1) ? true : false
      ...
      this.setState({
        ...
        flagCreateGroupPopup: flagCreateGroupPopup,
  ...
  onClickCreateGroup(type_popup) {
    let type = (type_popup === 'channel' ? 4 : 2)
    let groupsNChatChecked = this.state.groupsNChatChecked
              ...
              <div className='newgroup_rectangle_popup'>
                <div className='popup_choose_leader_title_area'>
                  <div className='popup_choose_leader_title'>
                    ...
                    {(this.state.typePopup === 'leave_group') && translate('Choose leader') }
                    {(this.state.typePopup === 'add_member') && translate('Add member') }
                    {(!this.state.flagCreateGroupPopup) && translate("New message") }
                    {(this.state.flagCreateGroupPopup && this.state.typePopup === 'create_group') &&  translate("Create a group")}
                    {(this.state.flagCreateGroupPopup && this.state.typePopup === 'create_channel') &&  translate("Create a channel")}
                  </div>
                  <div className='popup_choose_leader_btn_close'>
                    <img className='choose_leader_icon_close filter_img_class' alt='' src={changeColorSVG('./images/default/icons/icon-basic-up.svg')} onClick={e => this.onClickClosePopup()} />
                  </div>
                </div>
                ...
                {this.state.flagCreateGroupPopup && <div className="newgroup_wrap">
                  <div className="newgroup_camera_icon_place" onClick={(e) => { this.refs.data_image_camera.click() }}>
                    ...
                    {this.state.groupImageURL.length === 0 && <div className="newgroup_icon_area icon_camera" >
                      <img className="newgroup_icon_search filter_img_class" src={changeColorSVG('./images/default/icons/camera.svg')} alt="" />
                    </div>}
                ...
                <div className='popup_choose_leader_search_area'>
                  <div className='popup_choose_leader_icon_search_area'>
                    <img className='popup_choose_leader_icon_search filter_img_class' src={changeColorSVG('./images/default/icons/search-active.svg')} alt='' />
                  </div>
                ...
                {(!this.state.flagCreateGroupPopup && (['add_member', 'leave_group'].indexOf(this.state.typePopup) === -1)) &&
                  <div className='create_group_chat_area_popup'>
                    <img src={changeColorSVG('./images/default/icons/icon-basic-new-friend_group.svg')} alt='' className='avatar_create_group_chat_popup filter_img_class' />
                    ...
                    <div className='new_list_contacts_selected'>
                      {
                        arrUserSelected.map((ele, index) => (
                          <div key={ele.id} className='item-row-selected-user'>
                            ...
                            <img className='item-row-selected-close' src={changeColorSVG('./images/default/icons/icon-close.png')} alt='' onClick={e => { this.createNewChat(ele) }} />
                          </div>))
                      }
                    </div>
                  }
                ...
                {
                  this.state.flagCreateGroupPopup && (this.state.typePopup === 'create_group') &&
                  <div className='create-newgroup-row'>
                    <div className='create-newgroup-btn' onClick={e => this.onClickCreateGroup('group')}>
                      {translate("Create a group")}
                    </div>
                  </div>
                }
                {
                  this.state.flagCreateGroupPopup && (this.state.typePopup === 'create_channel') &&
                  <div className='create-newgroup-row'>
                    <div className='create-newgroup-btn' onClick={e => this.onClickCreateGroup('channel')}>
                      {translate("Create a channel")}
                    </div>
                  </div>
                }
                {
