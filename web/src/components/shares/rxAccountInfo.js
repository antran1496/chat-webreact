...
const { rxaget, rxChangeSlug, stringToColour, autoConvertTime, subString ,changeColorSVG, isLeaderCheck} = global.rootRequire('classes/ulti')
const { checkBlock, checkNameGroup, checkAvatarGroup, checkNameContact, checkNameUser, updateUsersOnline, getNumMemberOnl } = global.rootRequire('classes/chat')
...
class RxAccountInfo extends Component {
  ...
  checkMemberStatus(ele){
    let userid = this.userid
    let status_list = this.state.group.status_list
    if( ele.id.toString() === userid.toString()){
      return 1
    }else{
      for(let i = 0; i < status_list.length; i++){
        if(status_list[i].uin === ele.id.toString()){
            return status_list[i].online_status
        }
      }      
    }
    return 2
        ...
        <div className='account_info_tab_right' >
          <div className='account_info_tab_right-user'>
            <div className='account_info_tab_right-user-header'>
              ...
              <div className='account_info_tab_right-btn-edit'>
                ...
                */}
                {
                  (
                    this.isLeader(this.userid) && 
                    (rxaget(group, 'type', 0) !== 3 || rxaget(group, 'type', 0) !== 4 )
                  )
            ...
            <div className='account_info_tab_right-list-member' onClick={e => this.dropDownListMemberClick()}>
              <div className={((rxaget(group, 'type', 0) !== 3 && rxaget(group, 'type', 0) !== 4) || (rxaget(group, 'type', 0) === 4 && this.isLeader(this.userid)))? 'account_info_tab_right-list-member-body' : 'className_hiden'}>
          ...
          {/* HERE }
          {(rxaget(group, 'type', 0) === 3) && <div className='ztab-more-accountinfo'>
            {(this.user && this.user.phone.indexOf('ipa\\') !== -1) && <div className='ztab-more-accinfo_phone'>
              <span className='ztab-more-info_phone-txt'>{translate('Customer Id')}</span>
              <span className='ztab-more-info_phone_number'>{zget(this.users, [this.state.group.partner_id], {}).customerid || ''}</span>
            </div>}
            {<div className='ztab-more-accinfo_act'>
              {!this.state.isBlocked && <img className='icon32 icon-act filter_img_class' alt='' src={changeColorSVG('./images/default/icons/callright.svg')} onClick={(e) => this.onClickStartCall('voicecall')} />}
              {!this.state.isBlocked && <img className='videocall-icon icon-act filter_img_class' alt='' src={changeColorSVG('./images/default/icons/videocall.svg')} onClick={(e) => this.onClickStartCall('videocall')} />}
            </div>}
          </div>}
          ...
          <div className='ztab-add-member'>
            <div className='zact-add_account_info'>
              <span className = 
                {
                  ((rxaget(group, 'type', 0) !== 3 && rxaget(group, 'type', 0) !== 4 )|| 
                  (rxaget(group, 'type', 0) === 4) && this.isLeader(this.userid)) ? 
                  'zact-add-member' : 
                  'zact-add-member_hiden'}
          ...
          <div className='ztab-more-act'>
            <div className={(rxaget(group, 'type', 0) !== 3 && rxaget(group, 'type', 0) !== 4)? 'zact' : 'zact_hiden'}>
                ...
                {arrData.map((ele, index) => (
                  <div key={ele.media_uid + ele.url} className='zdocument-item'>
                    ...
                    {(tabactive === 'videos') && <div className='zvideo-list' onClick={e => this.showGallery({ index, type: 'video' })}>
                      ...
                      <div className='zvideo-item-play'><img src={changeColorSVG('./images/default/static/icon-media-play-white.svg')} alt='icon-media-play-white' /></div>
                    </div>}
          ...
          <div className='newManager_rectangle_1'>
            <div className='newManager_title_place'>
              ...
              <div className='newManager_btn_close'>
                <img className='newManager_icon_close filter_img_class' src={changeColorSVG('./images/default/icons/icon-basic-up.svg')} alt="" onClick={e => this.closeNewManager()} />
              </div>
            </div>
            <div className='newgroup_searchwrap'>
              <div className='newgroup_icon_search_area'>
                <img className='newgroup_icon_search_search filter_img_class' src={changeColorSVG('./images/default/icons/search-active.svg')} alt='' />
              </div>
