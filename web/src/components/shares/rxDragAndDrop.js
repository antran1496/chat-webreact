...
class rxDragAndDrop extends Component {
  ...
  handleDrop = (e) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if(typeNSupArr.length > 0){
        ...
        fileListArr = []
        displayListFileScreen = false
        let imageExtArr = [' jpg', ' gif', ' bmp', ' png', ' jpeg']
        let videoExtArr = [' m4v', ' avi', ' mpg', ' mp4']
        let docExtArr = [' xlsx', ' xls', ' doc', ' docx', ' ppt', ' pptx', ' txt', ' pdf', ' numbers',' key',' pages',' zip',' rar']
  ...
  uploadFileDoc(file) {
    ...
    if (typeof (file) !== 'undefined') {
      ...
      if (fileSize < (1024*1024*100)) {
        ...
        if (['rar'].indexOf(extFilename) !== -1) {
          fileTypeTmp = 'application/x-rar-compressed'  
        }

        if (['doc','docx'].indexOf(extFilename) !== -1) {
          fileTypeTmp = 'application/msword'
        }

        if (['ppt','pptx'].indexOf(extFilename) !== -1) {
  ...
  getFileType(filename){
    ...
    let imageExtArr = ['jpg', 'gif', 'bmp', 'png', 'jpeg']
    let videoExtArr = ['m4v', 'avi', 'mpg', 'mp4']
    let docExtArr = ['xlsx', 'xls', 'doc', 'docx', 'ppt', 'pptx', 'txt', 'pdf', 'numbers','key','pages','zip','rar','m4a','mp3']
                      ...
                      <div className ='zchat-drop-fileItem'>
                        <div className ='zchat-drop-fileImage_place'>
                          ...
                          {ele.type === 'video' && <div>
                            ...
                            <img className='zchat-drop-playicon' src={changeColorSVG('./images/default/static/icon-media-play-white.svg')} alt='icon-media-play-white' />
                            </div>
                          }
                          {ele.type === 'filedoc' && 
                        ...
                        <div className = 'zchat-drop-icon file-edit-icon' onClick={(e) => { this.refs['file_edit_icon' + index].click()}}>
                          ...
                          <input type='file' id={'file_edit_icon' + index} ref={'file_edit_icon' + index} multiple={false}
                            style={{ display: 'none' }}
                            accept="image/jpg,image/jpeg,image/png,video/mp4,.xlsx,.xls,.doc,.docx,.ppt,.pptx,.txt,.pdf,.numbers,.key,.pages,.zip,.rar,.m4a,.mp3"
