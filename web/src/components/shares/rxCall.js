  ...
  componentDidUpdate (prevProps, prevState) {
    if ((rxaget(this.props, 'netaCall.status', '') !== rxaget(prevProps, 'netaCall.status', '')) && ['voicecall', 'videocall'].indexOf(rxaget(this.props, 'netaCall.type_call', '')) !== -1) {
      ...
      if (isElectron()) {
        ...
        let remoteUser = {}
        let currentUser = {token: rxaget(this.props, 'netaauth.user.token', ''), id: this.userid}
        if (grouptmp && grouptmp.occupants_uins.constructor === Array && grouptmp.occupants_uins.length > 0) {
          let remoteUin = grouptmp.occupants_uins.filter(u => u !== this.userid.toString())[0]
          remoteUser = this.users[remoteUin] || {}
          if (remoteUser && remoteUser.id) {
            remoteUser['name_contact'] = (this.users[remoteUin]) ? checkNameUser(this.users[remoteUin]) : ''
          }
          
          currentUser = { ...this.users[this.userid], ...currentUser }
        }

        window.ipcRenderer.send('data_call', {
          typecall: typecall,
          grouptmp: grouptmp,
      ...
      if (rxio.connected) {
        if (!isElectron()) {
          ...
          rxio.socket.on('message', (data) => {
            if (this.callObj && this.callObj.call_id && data.attachments.indexOf(this.callObj.call_id) !== -1) {
              this.stopStream()
            }
          })
                  ...
                  if (!this.isRoomCreator) {
                    ...
                    this.rtcPeerConnection.onaddstream = function (e) { 
                      if (this.remoteAudioComponent) {
                        ...
                        try {
                          this.remoteAudioComponent = document.getElementById('remote-audio')  
                          this.remoteAudioComponent.srcObject = e.stream
                          this.remoteStream = e.stream  
                        } catch(e3) {}
                      }
        ...
        rxio.socket.on('start_call', async (data) => {
          if (isElectron()) {
            window.ipcRenderer.send('start_call', data)
          } else {
            if (!this.state.flagCalling) {
              ...
              try {
                ...
                this.setState({flagCalling: true, ringtone: true}, async () => {
                  ...
                  this.localAudioComponent = document.getElementById('local-audio')
                  this.remoteAudioComponent = document.getElementById('remote-audio')
