...
const {changeColorSVG} = global.rootRequire('classes/ulti')
...
class RxUploadFile extends Component {
  ...
  uploadFileDoc(fileList) {
    ...
    for (let i = 0; i < fileList.length; i++) {
      ...
      if (['xlsx', 'xls', 'doc', 'docx', 'ppt', 'pptx', 'txt', 'pdf', 'numbers', 'key', 'pages', 'zip', 'rar', 'm4a', 'mp3'].indexOf(ext) === -1) {
        checkExtFile = false
        break;
      }
    }
    if (checkExtFile) {                                  
        ...
        if (typeof (fileList[i]) !== 'undefined') {
          ...
          if (fileSize < (1024*1024*100)) {
            ...
            if (['rar'].indexOf(ext) !== -1) {
              fileTypeTmp = 'application/x-rar-compressed'  
            }

            if (['doc','docx'].indexOf(ext) !== -1) {
              fileTypeTmp = 'application/msword'
            }

            if (['ppt','pptx','key'].indexOf(ext) !== -1) {
              fileTypeTmp = 'application/vnd.ms-powerpoint'
            }
      ...
      alert(translate('The file format is not suitable. Please select the file as .xlsx, .xls, .doc, .docx, .ppt, .pptx, .txt, .pdf, .numbers, .key, .pages, .zip, .rar'))
    }
  }
  ...
  uploadFileImages(fileList) {
    let checkExtFile = true
    for (let i = 0; i < fileList.length; i++) {
      if (['png', 'jpg', 'jpeg'].indexOf(this.isImage(fileList[i]['name'])) === -1) {
        checkExtFile = false
        break;
      }
    }
    if (checkExtFile) {
      ...
      for (let i = 0; i < fileList.length; i++) {
        if (typeof (fileList[i]) !== 'undefined') {
          let file = fileList[i]
          let fileSize = fileList[i].size
          if (this.isImage(file.name) && ['png', 'jpg', 'jpeg'].indexOf(this.isImage(file.name)) !== -1) {
            let extFilename = 'png'
            ...
            fetch(global.rxu.config.create_blob, requestOptions).then(res => res.json()).then((json) => {
              if (json && json.blod_object && json.blod_object.form_data) {
                ...
                fetch(json.blod_object.params, { method: 'POST', body: dataUpload })
      ...
      alert(translate('The image file format is not suitable. Please select the image file as .jpg, .png, .jpeg'))
    }
  }
  ...
  render() {
    return (
      <div className='box-upload-wrap' ref={this.wrapperRef}>
        <img src={changeColorSVG('./images/default/static/plus.svg')} alt='icon-like' className='icon-upload-file icons-sticker filter_img_class'  onClick={(e) => this.toggleDataUpload()} />
        ...
        <input ref={(ref) => { this.uploadInputDoc = ref}} type='file' multiple={false} key={this.state.theInputFileKey + '_data_doc'} style={{ display: 'none' }} onChange={(e) => this.onChangeDataFile(e, 'doc')} accept=".xlsx,.xls,.doc,.docx,.ppt,.pptx,.txt,.pdf,.numbers,.key,.pages,.zip,.rar,.m4a,.mp3" />      
      </div>
    )
  }
}
