...
global.isclient && require('./rxCallScreen.css')
  ...
  return <div className='zchat_callscreen'>
    ...
    {vm.state.ringtone && <audio controls autoPlay hidden loop>
  	...
  	<div className='zchat_callscreen_main'>
  		<div className='zchat_callcenter_new'>
	        ...
	        <div className='zchat_callInfo'>
              ...
              {vm.state.flagStartCall && <div className='zcall_content'>
                ...
                <img className='zcall_avatarimg' src={`${global.rxu.config.cdn_endpoint}` + rxaget(objUsers, remote_uin+'.profile_url', '')} alt='profile_url' onError={(e) => {e.target.onerror = null; e.target.src = './images/default/static/avadefault.svg' }}/> 
              </div>
	            ...
	            <div className='zcall_buttons' id='zcall_buttons'>
                  ...
                  {vm.state.flagStartCall && <div className='zcall_button' onClick={e => vm.changeFlagMic()}>
                    <img className='zcall_btn zcall_btn_bg' alt='' src={changeColorSVG('./images/default/icons/mic_enable.svg')} style={(vm.state.flagMic) ? {display: 'block'} : {display: 'none'}} />
                    <img className='zcall_btn zcall_btn_border' alt='' src={changeColorSVG('./images/default/icons/mic_disable.svg')} style={(vm.state.flagMic) ? {display: 'none'} : {display: 'block'}}/>
                  </div>}
                  {(vm.state.flagStartCall && typeCall === 1) && <div className='zcall_button' onClick={e => vm.changeFlagCamera()}>
                    <img className='zcall_btn zcall_btn_border' alt='' src={changeColorSVG('./images/default/icons/call_video_enable.svg')} style={(!vm.state.flagCamera) ? {display: 'block'} : {display: 'none'}}/>
                    <img className='zcall_btn zcall_btn_bg' alt='' src={changeColorSVG('./images/default/icons/call_video_disable.svg')} style={(vm.state.flagCamera) ? {display: 'block'} : {display: 'none'}} />
                  </div>}
	    ...
	    <div className='zchat_videocallcenter_new' style={((typeCall === 2 || vm.state.flagCamera) && vm.state.flagStartCall) ? {display: 'block'} : {display: 'none'}}>
        ...
        <div className='zchat_videocallmain'>
          <div className='zchat_videocall_remote'>
            <video id="remote-video-screen" autoPlay="autoplay"></video>
            <audio id="remote-audio-screen" autoPlay></audio>
          </div>

          <div className='zchat_videocall_local'>
            <video id="local-video-screen" autoPlay="autoplay" muted="muted"></video>
            <audio id="local-audio-screen" autoPlay muted="muted"></audio>
          </div>
        </div>
