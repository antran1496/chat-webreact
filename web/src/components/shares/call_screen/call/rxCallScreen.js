...
import * as Jsx from './rxCallScreen.jsx'
...
const { rxaget, secondsToTime ,changeColorSVG} = global.rootRequire('classes/ulti')
...
class Component_ extends RxComponent {
  constructor(props, context) {
    ...
    this.sendConnected = this.sendConnected.bind(this)
  }
  ...
  loadDidMount() {
    ...
    if (window && window.ipcRenderer) {
      window.ipcRenderer.on('configs_call', (event, args) => {
        setTimeout(() => {
          this.onClickStartCall(args.typecall, args.grouptmp)  
        }, 300)
      })

      window.ipcRenderer.on('start_call_screen', (event, args) => {
        setTimeout(() => {
          let data = args
          if (!this.state.flagCalling) {
            ...
            try {
              ...
              this.setState({flagCalling: true, ringtone: true}, async () => {
                this.localVideoComponent = document.getElementById('local-video-screen')
                this.remoteVideoComponent = document.getElementById('remote-video-screen')

                this.localAudioComponent = document.getElementById('local-audio-screen')
                this.remoteAudioComponent = document.getElementById('remote-audio-screen')

                await this.setLocalStream(mediaConstraints, this.callObj.media_type)
              })
            } catch(e) {}
          }
        }, 300)
      ...
      window.ipcRenderer.on('close_call_screen', (event, args) => {
    ...
    rxio.getWaitConnect((data) => {
      if (rxio.connected) {
        rxio.socket.on('event', async (data) => {
          if (data.type === 5) {
            if (this.state.flagStartCall && this.callObj && this.callObj.media_type && this.callObj.media_type === 1) {
              ...
              this.setState({flagCamera: true}, () => {
                if (window && window.ipcRenderer) {
                  window.ipcRenderer.send('changesize_videocall', {type: 'videocall'})  
                }
          ...
          if (data.type === 6) {
            if (window && window.ipcRenderer) {
              window.ipcRenderer.send('changesize_videocall', {type: 'voicecall'})  
            }
          }
            ...
            if (this.state.flagStartCall) {
              if (event.sdp) {
                if (!this.isRoomCreator) {
                  ...
                  this.rtcPeerConnection.onconnectionstatechange = this.sendConnected
                  this.rtcPeerConnection.setRemoteDescription(new RTCSessionDescription({type: 'offer', sdp: event.sdp }))

                  this.rtcPeerConnection.onaddstream = function (e) { 
                    if (this.remoteAudioComponent) {
                      this.remoteAudioComponent.srcObject = e.stream
        ...
        rxio.socket.on('answer_call', (event) => {
          ...
          if (this.callObj && this.callObj.media_type && this.callObj.media_type === 2) {
            if (window && window.ipcRenderer) {
              window.ipcRenderer.send('changesize_videocall', {type: 'videocall'})  
            }
          }
        ...
        this.rtcPeerConnection.getSenders()
          .forEach(async (sender) => {
            if (sender && sender.track) {
              ...
              const newSender = this.rtcPeerConnection.addTrack(videoTrack, stream)
            }
          })
      })
    } catch(e2) {}  
  ...
  stopStream() {
    ...
    this.setState({ringtone: false, flagCalling: false, flagStartCall: false, isStarted: false, seconds: 0, flagMic: true, flagCamera: false, time: {h:'00',m:'00',s:'00'} }, () => {
      if (window && window.ipcRenderer) {
        window.ipcRenderer.send('stop_call', this.callObj)  
      }
    })
  }
  ...
  async setLocalStream(mediaConstraints, type) {
    let stream
    try {
    ...
    try {
      this.localStream = stream
      this.localVideoComponent.srcObject = stream
      this.localAudioComponent.srcObject = stream
    } catch(e) {}
  }
