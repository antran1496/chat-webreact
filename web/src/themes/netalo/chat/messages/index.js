...
const { timeConverter, timeToDate, secondToTime, rxaget, formatBytes, subString, changeTheme, changeTheme_ThemeColor, changeThemeColor, rxconfig, isElectron, sortArrObj, timesUnixToDate, convertSecondUnix, changeColorSVG} = global.rootRequire('classes/ulti')
const { updateUserStatus, netaGroupsRemove, netaBlobsUpdate, netaMessAdd, netaMessUpdate, netaMessPin, chooseGroupAction, netaUserUpdate, netaGroupsUpdate, netaGroupsUpdateLastMess, changeStatusTabmore, netaLinkAdd, netaMediaOneAdd,userClickUpdate, clickGroupCall, netaGroupCallRemove } = global.rootRequire('redux')
const { checkIsOwner, parseMessageDesc, checkNameUser, checkNameGroup, getDeleteMessStr, checkBlock } = global.rootRequire('classes/chat')
...
window.ClickInfoUser = (useridtmp,userOwnerid,groupid) => {
  if(useridtmp !== userOwnerid){
    ...
    let infoUserScreen = window.vmChat.state.infoUserScreen
  }
}
window.loadingFileCancel = (message_id)=>{
  let ele = document.getElementById(message_id);
  if(ele){
    ele.closest("div.zmessage_row").style.display="none";
  }
  window.vmChat.setState({cancelLoadingFile:true})
}
class Component_ extends RxComponent {
  constructor(props, context) {
    ...
    this.onClickGroupCall = this.onClickGroupCall.bind(this)

    window.vmChat = this //register vm
 
    let callGroupId = this.props.callGroupId
    if(callGroupId){
      let group = this.groups[callGroupId]
  ...
  componentDidMount() {
    ...
    if (rxio.socket) {
      rxio.socket.on('user_status', (data) => {
        if (data && data.user_status && data.user_status.uin) {
          this.props.updateUserStatus(data.user_status)  
        }
      })
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    try {
      if (this.users && Object.getOwnPropertyNames(this.users).length === 0) {
  ...
  loadHandleSocket() {
    rxio.getWaitConnect((data) => {
      if (data) {
          ...
          try {
            ...
            if (groupObj) {
              ...
              if (data.unblocked_all.length > 0 || data.blocked_all.length > 0) {
                ...
                let isBlocked = checkBlock(groupObj, this.userid)
        ...
        rxio.getWaitMessDel((data) => {
          this.props.netaMessUpdate(data.group_id, data)
          this.handleMessage(data, false)
        })
  ...
  loadDidMount() {
    ...
    window.onClickGroupCall = this.onClickGroupCall
  ...
  showGallery(strimages, index) {
    ...
    // let flagStartCall = this.props.flagStartCall
    // if (!flagStartCall) {
      let a = this.state.displaymodalpin
      let b = this.state.checkgallery
      let c = this.state.galleryImages
      let d = this.state.galleryPosition
  ...
  handleClickOutside(event) {
    ...
    if(className !== 'menu-option-forward'){     
      if (this.state.mess_id && event.button === 0 && event.target.className !== 'menu-option' ) {
        if ([1,2].indexOf(this.state.type_option) === -1) {
          this.setState({ 
            mess_selected: {}, 
            mess_id: '' 
          })  
            ...
            if (messages[datetime.toString()] && messages[datetime.toString()].data && messages[datetime.toString()].data.constructor === Array) {
              messages[datetime.toString()].data.forEach(mess => {
                if (mess.message_id === omess.message_id) {
                  mess.status = 4
                  this.props.netaMessUpdate(mess.group_id, mess)
                }
              })
      ...
      if (!isNaN(objmsg.type)) {
        ...
        if (objmsg.status !== 4) {
          if (objmsg.deleted_uins && objmsg.deleted_uins.constructor === Array && objmsg.deleted_uins.length > 0 && objmsg.deleted_uins.indexOf(userid) !== -1) {
            message = getDeleteMessStr(objmsg, this.users, this.userid)
          } else {
            let mess = ''
            switch (objmsg.type) {
              case 1:
              ...
              case 4:
                ...
                break
              case 5:
                if(objmsg.group_type === 4){
                  mess = translate(' created a channel')
                  message = checkNameUser(rxaget(this.users, objmsg.sender_uin.toString(), {})) + mess  
                }else if (objmsg.group_type !== 3) {
                  mess = translate(' created a group')
                  message = checkNameUser(rxaget(this.users, objmsg.sender_uin.toString(), {})) + mess  
                } else {
                  try {
                    let receiver_uin = objmsg.recipient_uins.filter(uin => uin.toString() != this.userid.toString())  
                    if (receiver_uin && receiver_uin.length > 0) {
                      mess = translate('Start chat with ')
                      message = mess + checkNameUser(rxaget(this.users, receiver_uin[0].toString(), {}))  
                    }
                  } catch(e3) {}
                }
                    ...
                    if (namesticker && typeof (namesticker) === 'string') {
                      ...
                      if (indexPath !== -1) {
                        if (this.checkSticker(stickerStr, imgsticker)) {
                          message = `<img src='${this.state.pathImg}/${stickerStr}/${imgsticker}' alt='sticker' class='img-sticker' onerror='this.onerror=null;this.src="./images/tabs/default.jpg";'/>`
                        } else {
              ...
              case 12:
                ...
                if (file) {
                  ...
                  let fileNameExt = file_name + '.'+ rxaget(file, 'file_extension', '')
              ...
              case 16:
                ...
                if (zget(this.props, 'themeValue') === 'blueColor') {
                  ...
                  if (rxconfig.theme === 'galo') {
                    ...
                    stylecallgroup = '#fce4d8'
                  }
                }

                let objCallStr = JSON.stringify({
                  group_id: objmsg.group_id,
                  message_id: objmsg.message_id
                })

                message = `<div class='zchat-callmess' style='background: ${stylecallgroup};'>
                  <div class='zchat-callname'>${translate('Call Group')}</div>`
                  
                  const time = Number(rxaget(attachments, 'stopped_at', 0)) - Number(rxaget(attachments, 'created_at', 0))
                  message += `<div class='zchat-calltime'><i class='icon-call-in' style='color:#2fb13e;'></i>${secondToTime(time)}</div>`
                
                message += `<span class='zchat-callicon' onclick=window.onClickGroupCall(${objCallStr})><i class='icon-phone'></i></span></div>`
                if (type === 'desc') {
                  mess = translate(' finished')
                  message = call_name + mess
                }
                    ...
                    if (urls && urls.constructor && urls.length > 0) {
                      ...
                      if (objLink && !objLink.uid) {
                        ...
                        for (let link of urls) {
                          messagetmp = messagetmp.replace(link, `<span class='highlight_link' onclick=window.openExternalLink('${link}')>${link}</span>`)
                        }
  ...
  chooseGroup(group) {
    ...
    this.setState({popupGroup: false, group_id: group_id, group: group, messages: objMessTmp, pinedMess: objMessPin, scroll_mess: false, mess: '' }, () => {
      ...
      if (rxio.connected) {
        ...
        rxio.socket.emit('list_message', msg, (data) => {
          let netaGroupJoinCall = global.rxu.json(rxgetLocal('netaGroupJoinCall'), { status: false, groups: {} })
          data.messages.forEach(oMess => {
            if (oMess.type === 6 && netaGroupJoinCall['groups'] && netaGroupJoinCall['groups'][oMess.group_id] && oMess['attachments'].indexOf(netaGroupJoinCall['groups'][oMess.group_id]['message_id']) !== -1 ) {
          ...
          if (group_id === this.state.group_id) {
            if (data && data.last_message_id && data.messages && data.messages.constructor === Array && data.messages.length > 0) {
              ...
              this.setState({ messages: objDate, scroll_mess: true }, () => {
                ...
                let lassMessObj = data.messages[data.messages.length - 1]
                if (lassMessObj.message_id !== last_message_id_tmp) {
                  this.checkReaded(lassMessObj.group_id, lassMessObj.message_id)
                  this.props.netaGroupsUpdateLastMess(lassMessObj.group_id, lassMessObj.message_id, lassMessObj)
                }
              })
            ...
            if (groups && groups.constructor === Array && groups.length > 0) {
              groups.forEach(group => {
                if (group.group_id === group_id) {
                  ...
                  if (group && group.last_message && group.last_message.message_id) {
