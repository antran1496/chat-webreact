...
/*global translate*/
const { timeConverter, rxaget, rxChangeSlug, stringToColour, idToColor ,changeColorSVG} = global.rootRequire('classes/ulti')
    ...
    <div id='menu-message'> 
      {(vm.state.mess_id !== '') && <div className={(vm.checkTypeDelMess() === 4) ||([1,10].indexOf(rxaget(vm.state, 'mess_selected.type', '')) !== -1 && rxaget(vm.state, 'mess_selected.sender_uin', '') !== vm.userid.toString()) ? 'menu-message-box_hiden' : 'menu-message-box'}>
        {vm.checkTypeDelMess() === 3 &&
          <div>
            {(vm.state.isBlocked === 0) && <div className='menu-option' onClick={e => vm.chooseOption(e, 'reply')}>{translate('Reply')}</div>}
            ...
            {(vm.checkPinMess() === 1 && vm.state.isBlocked === 0) &&
              <div className='menu-option' onClick={e => vm.chooseOption(e, 'pin')}>{translate('Pin')}</div>}
            {(vm.checkPinMess() === 2 && vm.state.isBlocked === 0) &&
              <div className='menu-option' onClick={e => vm.chooseOption(e, 'unpin')}>{translate('Unpin')}</div>}
          </div>}
        {(rxaget(vm.state, 'mess_selected.sender_uin', '') === vm.userid.toString() && vm.state.isBlocked === 0) && <div className='menu-option' onClick={e => vm.chooseOption(e, 'delete_all')}>{translate('Delete with everyone')}</div>}
        {(rxaget(vm.state, 'mess_selected.sender_uin', '') === vm.userid.toString() && vm.state.isBlocked === 0) && <div className='menu-option' onClick={e => vm.chooseOption(e, 'delete_one')}>{translate('Delete only me')}</div>}
      </div>}
