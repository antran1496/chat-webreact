...
export default (props) => {
  ...
  const SortableItem = SortableElement(({ omess, i, arr }) => {
    let flagShowName = ((i > 0 && omess.sender_uin !== arr[i - 1].sender_uin) || i === omess.firstMessageIndex) && ((props.group.occupants_uins.length > 2) && (props.userid.toString() !== omess.sender_uin.toString()))
    return (<div key={omess.message_id} style={{ cursor: isSort ? "pointer" : "inherit" }}
          ...
          <div className={checkIsOwner(omess, props.userid) ? 'zmessage_content_right' : 'zmessage_content'}>
            <div className='zmessage_box'>
              <div className={(omess.type === 0) ? (props.userid.toString() === omess.sender_uin.toString())
                ...
                {(flagShowName === true) && <div id='stsender_name' className={(omess.type !== 0) ? (omess.type === 9 ? ' zsender_name_type9 ' : 'zsender_name_basic mess_type0') : 'zsender_name_basic'}>
      ...
      {isSort && <div>
        <img className='icon24 zuitem-right' src={changeColorSVG('./images/default/icons/sort.svg')} alt='' />
        </div>}
    </div>)
  });
