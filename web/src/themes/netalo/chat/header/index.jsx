...
const { rxChangeSlug, stringToColour, autoConvertTime, subString ,changeColorSVG} = global.rootRequire('classes/ulti')
    ...
    <div>
      <div className='ztbar'>
        ...
        <div className='ztbar_right'>
          ...
          {(zget(vm.state.group, 'isDraft', 0) != true && zget(vm.state.group, 'type', 0) === 3 && !vm.state.isBlocked) && <div className='ztbar_btn ztbar_btncall filter_img_class' onClick={(e) => vm.onClickStartCall('voicecall')}><img src={changeColorSVG('./images/default/icons/callright.svg')} alt='' className='icon-phone icon32' /></div>}
          {(zget(vm.state.group, 'isDraft', 0) != true && zget(vm.state.group, 'type', 0) === 3 && !vm.state.isBlocked) && <div className='ztbar_btn ztbar_btnsearch filter_img_class' onClick={(e) => vm.onClickStartCall('videocall')}><img src={changeColorSVG('./images/default/icons/videocall.svg')} alt='' className='icon-magnifier_icon32' /></div>}

          {(vm.state.isShowBtnGroupCall && zget(vm.state.group, 'type', 0) === 2 && !vm.state.isBlocked) && <div className='ztbar_btn ztbar_btnsearch filter_img_class' onClick={(e) => vm.onClickStartGroupCall('groupcall')}><img src={changeColorSVG('./images/default/icons/videocall.svg')} alt='' className='icon-magnifier_icon32' /></div>}

          <div className={!(vm.state.tabmore === true || vm.state.tabmore.data === true) ? 'ztbar_btn ztbar_btnmore' : 'ztbar_btn  ztbar_btnmore ztbar_btn-active'} onClick={e => vm.onClickTabMore()}><img src={changeColorSVG('./images/default/icons/menumore.svg')} alt='' className='icon-options icon32' /></div>
        </div>
      </div>
      { (vm.state.tabmore === true || vm.state.tabmore.data === true) ? <RxAccountInfo /> : null}
      {!vm.state.isShowBtnGroupCall && <div className='ztbar-groupcall'>
        <div className='ztbar_left-groupcall'>
          <div className='ztbar_name-groupcall'>{translate('Group Call')}</div>
          <div className='ztbar_desc-groupcall'>{translate('Ongoing Group Call')}</div>
        </div>
        <div className='ztbar_right-groupcall' onClick={(e) => vm.onClickJoinGroupCall()}>
          {translate('Join')}
        </div>
      </div>}
