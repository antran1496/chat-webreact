...
const { secondToTime, rxaget, formatBytes, rxChangeSlug, subString, changeColorSVG, isLeaderCheck} = global.rootRequire('classes/ulti')
const {chooseGroupAction, netaMessAdd,updateRecentContactGroups,netaMediaOneAdd, netaGroupsUpdateMessDraft,netaGroupsRemove,netaMessRemove, removeGroupsDraft } = global.rootRequire('redux')
...
class Component_ extends RxComponent {
  constructor(props, context) {
    ...
    this.netaGroupsDraft = rxaget(this.props, 'netaGroupsDraft', {})
  ...
  UNSAFE_componentWillReceiveProps(nextProps) { 
    if(this.state.cancelLoadingFile !== nextProps.cancelLoadingFile){
      let cancelLoadingFile = nextProps.cancelLoadingFile
      
      this.setState({cancelLoadingFile:cancelLoadingFile})
    }
    ...
    if (this.state.group) {  
      ...
      } else if (groupObj.isDraft && this.props.rxgroup && this.props.rxgroup.group && this.props.rxgroup.group.group_id == groupObj.group_id && typeof(this.props.rxgroup.group.isDraft) === 'undefined') {
        groupObj = rxaget(this.props, 'rxgroup.group', {})
      }
      ...
      if (groupObj.group_id !== nextProps.group_id) {
        const chatDiv = document.getElementById('zmess-box')
        if (chatDiv) {
          if (this.state.mess.length > 0 ) {
            if (groupObj.isDraft) {
              const draft_mess = {
                'created_at': new Date().getTime() * 1000,
                'message': this.state.mess,
                'group_fullname': (groupObj.fullnamegroup) ?  groupObj.fullnamegroup : groupObj.group_fullname || ''
              }
              this.props.netaGroupsUpdateMessDraft(groupObj.group_id,draft_mess)  
            }    
          } else {
            if(this.netaGroupsDraft && this.netaGroupsDraft.groups && groupObj.group_id && this.netaGroupsDraft.groups[groupObj.group_id]){
              this.deleteGroup(groupObj)
              this.props.removeGroupsDraft(groupObj.group_id) 
            } else {
              if (nextProps && nextProps.netaGroups && nextProps.netaGroups.groups && nextProps.netaGroups.groups[groupObj.group_id] &&  nextProps.netaGroups.groups[groupObj.group_id]['isDraft']) {
                this.deleteGroup(groupObj)
                this.props.removeGroupsDraft(groupObj.group_id)  
              }
            }
          ...
          const groupCheckDraft = nextProps.rxgroup.group
          if(groupCheckDraft.draft_mess && groupCheckDraft.draft_mess.message && groupCheckDraft.draft_mess.message.length > 0){
            chatDiv.innerHTML = groupCheckDraft.draft_mess.message
          }else{
            chatDiv.innerHTML = ''
          }
          this.setState({ 
  ...
  handleContentEditable = event => {
    try {
      ...
      if(value.length === 0){
        this.setState({displayLikeIcon:true})
      }else{
        if(this.state.displayLikeIcon){
          this.setState({displayLikeIcon:false})
        }
      }
      if (value && value.match(/<span(.*?)<\/span>/g) && value.match(/<span(.*?)<\/span>/g).length > 0) {
        value.match(/<span(.*?)<\/span>/g).map((val) => {
          ...
          if (arrid && arrid[0]) {
            ...
            if (cont && tagNameDisplay[uid] && tagNameDisplay[uid]['name'] && cont !== tagNameDisplay[uid].name) {
      ...
      if (this.userid && group_id) {
        ...
        if (strvalue) {
          ...
          strvalue = strvalue.replace(/\&amp\;/g,'&').replace(/\&lt\;/g,'<').replace(/\&gt\;/g,'>')
          ...
          this.setState({
            ...
            this.props.netaGroupsUpdateMessDraft(group_id,{})
          })
        }
      }
  ...
  disableNewlines = event => {
    ...
    if (event && keyCode === 13 && event.shiftKey) {
      ...
      let group_id = this.state.group.group_id

      if (this.netaGroupsDraft && this.netaGroupsDraft.groups && this.netaGroupsDraft.groups[group_id]) {
        this.removeDraftGroup(this.state.group)
      } else {
        if (this.state.group && !this.state.group.isDraft) {
          this.sendMessClick()  
        } else {
          this.removeDraftGroup(this.state.group)
        }
  ...
  removeDraftGroup(group){
    ...
    rxio.checkGroup(params,(data) => {
      ...
      console.log(data)
      if(!data.group){
        this.createNewChat(group.partner_id, group)
      }else{
        if (group.isDraft) {
          this.createNewChat(group.partner_id, group)
        } else {
      ...
      // this.props.removeGroupsDraft(group.group_id) 

    })
  }

  createNewChat(partner_id, groupDraft) {
    ...
    const params = {
      ...
      'name': '',
      'avatar_url': '',
      'occupants_uins': [userOwner.id.toString(),partner_id],
    ...
    try {
      if (partner_id) {
        try {
          ...
          let messTmp = this.state.mess

          rxio.createNewGroup(params, (data) => {
            if (data && data.group) {
              if (data.group.type === 3) {

                this.props.removeGroupsDraft(groupDraft.group_id) 
                this.props.netaGroupsRemove(groupDraft.group_id)
                this.props.netaMessRemove(groupDraft.group_id)
                this.props.chooseGroupAction(data.group)   


                data.group.partner_id = partner_id
                this.sendMessClick(data.group.group_id, messTmp)
              }         
            }
          })
        } catch(e1) {
  ...
  sendMessClick(groupIdTmp, messTmp){
      const imageTmp = this.state.imageTmp || {}

      let group_id = groupIdTmp || this.props.group_id
      ...
      if (imageTmp && imageTmp.base64) {  
        ...
        if (displayTagNameBox === true && groupsTemp && groupsTemp.length > 0 && this.state.tagNameSelect ){          
          ...
          //   event.preventDefault()
            let mess = messTmp || this.state.mess      
            ...
            if(this.state.type_option === 2 || emptyMess.length > 0 ){
              ...
              if (group_id && (this.state.type_option === 2 || typeof mess !== 'boolean')) {
                ...
                if (rxio.connected) {
                  ...
                  rxio.socket.emit('create_message', msg, (data) => {
                    if (data && data.message) {
                      this.setState({ type_option: 0, mess_selected: {}/*, mess: ''*/,displayTagNameBox: false,tagNameDisplay: {}, me_typing: false, displayLikeIcon: true}, () => {
  ...
  isLeader(eleId){
    let group = this.state.group
    return isLeaderCheck(eleId,group)
  }
...
const mapStateToProps = (state, ownProps) => ({
  ...
  netaGroupsDraft: state.netaGroupsDraft
})
