...
const { rxaget, autoConvertTime, rxChangeSlug, stringToColour ,changeColorSVG} = global.rootRequire('classes/ulti')
    ...
    <div className='zleft_contain'>
        ...
        <div className='dropdown'>
          ...
          { <div className='dropdown-content'>
              <div className='rectangle_1827'>
                <div className = 'dropdown-item' onClick={e => { vm.createNewChatGroup('create_group') }} >
              ...
              <div className='rectangle_1827'>
                <div className = 'dropdown-item channel'  onClick={e => { vm.createNewChatGroup('create_channel') }} >
                  <img  className='filter_img_class' src={changeColorSVG('./images/default/icons/icon-basic-new-friend_group.svg')} alt=''/>
                  <span className='new_chat'>
      ...
      <div className='zleft_contactlist' id='zgroup_list' onScroll={vm.scrollGroupsChat}>
        <div>
          { vm.state.groupsDisplay.length > 0 ? (
                ...
                return(<div className={(ele.group_id === vm.state.group_id) ? 'zgroupitem active clearfix' : 'zgroupitem clearfix'} key={index} onClick={e => { vm.chooseGroup(ele) }} onContextMenu={e => {vm.onRightClickGroup(e, ele)}}>             
                  <div className='zgroup_avatar'>                      
                    {ele.avatar_url 
                      ...
                      (vm.users[ele.partner_id] && vm.users[ele.partner_id]['profile_url'] 
                        ...
                        onError={(e) => {e.target.onerror = null; e.target.src = './images/default/static/avadefault.svg' }} /> 
                         : vm.checkImage[linkava + ele.partner_id] && <img src={vm.checkImage[linkava + ele.partner_id]['link']} alt='linkava' className='ava-usergroup images-static' onError={(e) => {e.target.onerror = null; e.target.src = './images/default/static/avadefault.svg' }} />
                      )
                    }
                    {
                  ...
                  <div className='zgroup_maininfo'>                                  
                    <div className={vm.state.statusInput===''?'zgroup_name':'zgroup_name_hiden'}>
                      { ele.type === 4 && <img className ='zgroup_icon ' src='./images/default/static/channel-icon.svg' alt='' />}
                      { ele.type === 2 && <img className ='zgroup_icon ' src='./images/default/static/group-icon.svg' alt='' />}
                    ...
                    <div className={vm.state.statusInput===''?'zgroup_desc':'zgroup_desc_hiden'}>
                      ...
                      <div className='zgroup_messageDesc'>
                      {
                        ele.draft_mess && ele.draft_mess.message && ele.draft_mess.message.length > 0 ?
                        (<div className='zgroup_messageDraftDesc'><span className='zgroup_DraftDescTitle'>Draft: </span> {ele.draft_mess.message}</div>) :
                        ele.isDraft ? (<div className='zgroup_messageDraftDesc'><span className='zgroup_DraftDescTitle'>{"Draft"}</span></div>) : parseMessageDesc(rxaget(ele, 'last_message', {}), vm.users, vm.userid)
                      } 
                      </div>
                    ...
                    <div className={vm.state.statusInput===''?'zgroup_time':'zgroup_time_hiden'}>
                      <span className='zgroup_time_checked'>
                        {vm.checkGroupSeen(ele) ? <img className ='filter_img_class' src={changeColorSVG('./images/default/static/icon-checked.svg')} alt='' /> : ''} 
                      </span>
                      <span className='zgroup_time_last'>
                        {
                          ele.draft_mess && ele.draft_mess.message && ele.draft_mess.message.length > 0 ? 
                          autoConvertTime(rxaget(ele, 'draft_mess.created_at', '')) :
                          (
              ...
              <div className={vm.state.statusInput===''?'zgroup-emty':'zgroup-emty_hiden'}>
                <img className='icon176x80 filter_img_class' src={changeColorSVG('./images/default/icons/backgroundchat.svg')}alt=''/>
        ...
        <div id='context_menu-group'>
          {vm.state.groupChooseItem_selected.isDraft !== true && vm.state.group_id !== '' && !(vm.netaGroupsDraft && vm.netaGroupsDraft.groups && vm.netaGroupsDraft.groups[vm.state.group_id]) &&(
            <div className={'Rectangle-1773'/*vm.checkTypeChooseGroup() === 3 ? 'context_menu-group-box' : 'context_menu-group-box_hiden'*/}>
             
              <div className={vm.state.groupChooseItem_selected.type !== 4 ?'group-menu_item border-bottom':'className_hiden'}>
                <div className ='group-item-head'>
                  <img className='group_img_item filter_img_class' src={changeColorSVG('./images/default/icons/Group 10@3x.svg')}alt=''/>
                </div>
              ...
              <div className={vm.state.groupChooseItem_selected.type !== 4 ?'group-menu_item border-bottom':'className_hiden'} onClick={e => vm.groupChooseItem(e, 'Mark as read')}>
                <div className ='group-item-head'>
                  <img className='group_img_item filter_img_class' src={changeColorSVG('./images/default/icons/icon-chat-readed.svg')}alt=''/>
                </div>
              ...
              <div className={vm.state.notificationStatus === true ?'group-menu_item border-bottom':'group-menu_item_hiden'} onClick={e => vm.groupChooseItem(e, 'Turn off notifications')}>
                <div className ='group-item-head'>
                  <img className='group_img_item group_img_item_head filter_img_class' src={(!vm.state.notificationStatus) ? changeColorSVG('./images/default/icons/icon-unnotifi.svg') : changeColorSVG('./images/default/icons/icon-notifi.svg')} alt=''/>
                </div>
              ...
              <div className={vm.state.groupChooseItem_selected.type !== 4 ?'group-menu_item border-bottom':'className_hiden'} onClick={e => vm.groupChooseItem(e, 'Pin a conversation')}>
                <div className ='group-item-head'>
                  <img className='group_img_item group_img_item_head filter_img_class' src={(!vm.checkGroupsPinned_uins(vm.state.groupChooseItem_selected.pinned_uins)) ? changeColorSVG('./images/default/icons/icon-pin.svg') : changeColorSVG('./images/default/icons/icon-unpin.svg')} alt=''/>
                </div>
              ...
              {/* <div className='group-menu_item border-bottom' onClick={e => vm.groupChooseItem(e, 'Hide a conversation')}>
                <div className ='group-item-head'>
                  <img className='group_img_item filter_img_class' src={changeColorSVG('./images/default/icons/icon-hide-chat.svg')}alt=''/>
                </div>
                <div className='group-item-info'>{translate('Hide a conversation')}</div>
                <div className='group-item-footer'>
                  <img className='group_img_item filter_img_class' src={changeColorSVG('./images/default/icons/icon-unhide-chat.svg')}alt=''/>
                </div>
              </div> */}

              <div className={vm.state.groupChooseItem_selected.type !== 4 ?'group-menu_item border-bottom':'className_hiden'}>
                <div className ='group-item-head'>
                  <img className='group_img_item filter_img_class' src={(vm.state.groupChooseItem_selected.type !== 3) ? changeColorSVG('./images/default/icons/ic_reply.svg') : changeColorSVG('./images/default/icons/icon-delete-chat.svg')}alt=''/>
                </div>
              ...
              {/* <div className='group-menu_item ' onClick={e => vm.groupChooseItem(e, 'Delete')}>
                <div className ='group-item-head'>
                  <img className='group_img_item filter_img_class' src={changeColorSVG('./images/default/icons/icon-delete-chat.svg')}alt=''/>
                </div>
          ...
          {vm.state.groupChooseItem_selected.isDraft === true && vm.state.group_id !== '' && !(vm.netaGroupsDraft && vm.netaGroupsDraft.groups && vm.netaGroupsDraft.groups[vm.state.group_id]) &&(
            <div className={'Rectangle-1773'/*vm.checkTypeChooseGroup() === 3 ? 'context_menu-group-box' : 'context_menu-group-box_hiden'*/}>
              <div className={vm.state.groupChooseItem_selected.type !== 4 ?'group-menu_item border-bottom':'className_hiden'}>
                ...
                <div className='group-item-info' onClick={e => vm.removeGroupsDraft(vm.state.groupChooseItem_selected.group_id)}>
                  {translate('Delete conversation Draft') }
                </div>
              </div>
            </div>
          )}
