...
const { rxaget, rxChangeSlug, sortArrObj ,changeColorSVG} = global.rootRequire('classes/ulti')
...
const { netaGroupsUpdate, netaGroupsUpdateArr, chooseGroupAction, userAdd, userAddArr, contactAdd, netaBlobsUpdate, 
      ...
      netaGroupsPinUpdate, clearRxContact, netaGroupsUpdateUnpin, clickPopup, netaMessGet, netaMessAdd, netaUserSetGroupid, netaMessRemove, updateUserStatus,
      removeGroupsDraft  } = global.rootRequire('redux')
...
class Component_ extends RxComponent {
  constructor(props, context) {
    ...
    const groupDraft = this.netaGroupsDraft

    if (groupDraft && groupDraft.groups) {
      for (const idgroup of Object.keys(groupDraft.groups)) {
        if (groupDraft.groups[idgroup]) {
          let ogroup = groupDraft.groups[idgroup]
          ...
          this.props.netaGroupsUpdate(ogroup.group_id, ogroup)
        }
      }
    }
  ...
  checkDraftMess(group){
    if(group.draft_mess && group.draft_mess.message && group.draft_mess.message.length > 0){
      return group.draft_mess.created_at
    }
  ...
  reSortGroup(netaGroups) {
    ...
    if (netaGroups.groups) {
      ...
      this.netaGroupsDraft = rxaget(this.props, 'netaGroupsDraft.groups', {})
      
      Object.keys(this.netaGroupsDraft).forEach(keygroup => {
        if (!groupsTmp[keygroup] && this.netaGroupsDraft[keygroup]) {
          if (this.netaGroupsDraft[keygroup].last_message) {
            try {
              arrGroupsTmp.push(this.netaGroupsDraft[keygroup])
            } catch (error) {}
          }
        }
      })
      ...
      let arrGroups = arrGroupsTmp.sort((a, b) => (this.checkDraftMess(a) > this.checkDraftMess(b)) ? -1 : ((this.checkDraftMess(b) > this.checkDraftMess(a)) ? 1 : 0))
      ...
      let groupsDisplay = arrGroups.slice(0, index+1)
  ...
  loadHandleSocket() {
    rxio.getWaitConnect((data) => {
      if (data) {
        ...
        //   try {
        //     if (data && data.seen_uins && data.seen_uins.constructor === Array && data.seen_uins.length > 0 && data.seen_uins.indexOf(this.userid.toString()) !== -1 ) {
  ...
  createNewChatGroup(type_popup) {
    ...
    if( tabmore.data === true){
      ...
      this.setState({ flagShowDropDown: false }, () => {
        ...
        this.props.clickPopup(type_popup, { data: { groups: this.state.groupsNChatOrigin, user: user } }, (data) => {
          this.fetchGroups()
        })
      })
    }else{
      ...
      function excefletch(pageIndex) {
        if (rxio.connected) {
          ...
          rxio.socket.emit('list_group', msg, (data) => {
            if (data && data.groups && data.groups.constructor === Array && data.groups.length > 0) {
              ...
              for (const ogroup of groups) {
                if (ogroup && ogroup.last_message) {
                  ...
                  let groupIdCurrent = (that.state && that.state.group_id) ? that.state.group_id : ''
                  if (ogroup.group_id !== groupIdCurrent) {
                  ...
                  if (ogroup.status_list && ogroup.status_list.constructor === Array && ogroup.status_list.length > 0) {
                    ogroup.status_list.forEach(oUser => {
                      objUserStatic[oUser.uin] = oUser
                    })
                  }
                }
              }

              if (Object.keys(objUserStatic).length > 0) {
                Object.keys(objUserStatic).forEach(kUser => {
                  that.props.updateUserStatus(objUserStatic[kUser])  
                })
              }
            ...
            } else if (data && data.groups && data.groups.constructor === Array && data.groups.length == 0) {
              that.props.netaGroupsUpdateArr([], true)
            }
          })
        }
      }
  ...
  removeGroupsDraft(groupid) {
    this.props.removeGroupsDraft(groupid) 
    this.props.netaGroupsRemove(groupid)
    this.props.netaMessRemove(groupid)
    this.props.chooseGroupAction({group_id: ''})
  }
