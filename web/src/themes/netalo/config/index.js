...
const { netaUpdateProfileAction, setComp, rxnavToggle, rxnavClose, netaloginAction, netalogoutAction, saveLanguage, savePage, saveTheme, saveThemeBackgroud , saveNotificationStatus, chooseGroupAction} = global.rootRequire('redux')
const rxio = global.rootRequire('classes/socket').default
const { rxaget ,changeTheme, changeThemeColor,changeTheme_ThemeColor,changeColorSVG} = global.rootRequire('classes/ulti')
  ...
  onClickUnBlockContact() {
    try {
      ...
      for (let key in unblockList) {
        let user = unblockList[key]
        let group = this.groups[user.group_id]
        
        rxio.unblockUser({ group_id: group.group_id, uni: user.uin}, () => {
          ...
          let blockedList = JSON.parse(JSON.stringify(that.state.blockedList))
          let index = blockedList.findIndex(i => i.uin.toString() === user.uin.toString())
          if (index !== -1) {
            blockedList.splice(index, 1)
          }
        ...
        .then((json) => {
          fetch(global.rxu.config.base_api_neta + '/api/users/' + this._user.id, {
            ...
            if (jsonb && jsonb.Id) {
              ...
              this.setState({ netaauth_profile_url: jsonb.profile_url,imgkey:'',displayEditScreen:false })
