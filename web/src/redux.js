...
let initialState = {
  ...
  netaGroupJoinCall: { status: false, groups: {} },
  ...
  netaGroupCall: { status: false, type_call: '', payload: {} },
  ...
  userClickInfo:{},
  netaGroupsDraft: { status: false, groups: {} }
}
...
const initialStateLate = {
  ...
  netaGroupJoinCall: global.rxu.json(rxgetLocal('netaGroupJoinCall'), { status: false, groups: {} }),
  ...
  netaGroupsDraft: global.rxu.json(rxgetLocal('netaGroupsDraft'), { status: false, groups: {} }),
}
...
export const updateGroupsDraft = (data,id,action) => ({
  type: 'UPDATE_GROUPS_DRAFT',
  data: data,
  id: id,
  action:action
})
export const removeGroupsDraft = (id)=> ({
  type: 'REMOVE_GROUPS_DRAFT',
  id: id,
})
...
export const netaGroupsUpdateMessDraft = (id, data) => ({
  type: 'NETA_GROUPS_UPDATE_MESS_DRAFT',
  id: id,
  data: data
})

export const netaGroupCallUpdate = (id, data) => ({
  type: 'NETA_GROUP_CALL_UPDATE',
  id: id,
  data: data
})

export const netaGroupCallStatusUpdate = (id, data) => ({
  type: 'NETA_GROUP_CALL_STATUS_UPDATE',
  id: id,
  data: data
})

export const netaGroupCallRemove = (id, data) => ({
  type: 'NETA_GROUP_CALL_REMOVE',
  id: id,
  data: data
})
...
export const netaMessUpdate = (groupid, mess) => ({
  type: 'NETA_MESS_UPDATE',
  id: groupid,
  mess: mess
})
...
export const clickGroupCall = (typecall, payload) => ({
  type: 'CLICK_GROUP_CALL',
  type_call: typecall,
  payload: payload
})
...
export const netaGroupCall = (state = initialState.netaGroupCall, action) => {
  switch (action.type) {
    case 'CLICK_GROUP_CALL': {
      if (!state) { state = { status: false, type_call: action.type_call || '', payload: action.payload || {} } }
      return { status: !state.status, type_call: action.type_call || '', payload: action.payload || {} }
    }
...
export const netaPopup = (state = initialState.netaPopup, action) => {
  switch (action.type) {
    case 'CLICK_POPUP': {
      ...
      if (['create_group', 'add_member', 'leave_group', 'create_channel'].includes(action.type_popup)) {
...
export const netaauth = (state = initialState.netaauth, action) => {
  switch (action.type) {
    ...
    case 'NETALOGOUT': {
      ...
      rxsetLocal('netaPage', '')
      rxsetLocal('netaGroupsDraft', JSON.stringify({}))
      return newState
    }
...
const mergeObj = (obj1, obj2) => {
  let keyObj = [{key: 'admin_uins', default: []}, {key: 'avatar_url', default: ''}, {key: 'background', default: ''}, {key: 'count_unread', default: 0}, {key: 'group_fullname', default: ''}, {key: 'blocked_uins', default: []}, {key: 'created_at', default: ''}, {key: 'creator_uin', default: ''}, {key: 'description', default: ''}, {key: 'group_id', default: ''}, {key: 'group_name', default: ''}, {key: 'last_active_messages', default: []}, {key: 'last_message', default: {}}, {key: 'last_message_id', default: ''}, {key: 'last_messages', default: []}, {key: 'last_sos_messages', default: []}, {key: 'locations', default: []}, {key: 'mode', default: 0}, {key: 'muted_uins', default: []}, {key: 'name', default: ''}, {key: 'occupants_uins', default: []}, {key: 'owner_uin', default: ''}, {key: 'pinned_messages', default: []}, {key: 'pinned_uins', default: []}, {key: 'received_list', default: []}, {key: 'removed_uins', default: []}, {key: 'secrets', default: []}, {key: 'seen_list', default: []}, {key: 'status_list', default: []}, {key: 'timestamp', default: 0}, {key: 'type', default: 0}, {key: 'unread_cnt', default: 0}, {key: 'updated_at', default: ''}]
  for (let okey of keyObj) {
    obj1[okey.key] = obj2[okey.key] || okey.default
  }
}
...
export const netaGroups = (state = initialState.netaGroups, action) => {
  ...
  switch (action.type) {
        ...
        for (const obj of action.data) {
          if (obj.group_id) {
            if (obj.isDelete) {
              ...
              if (state.groups && !state.groups[obj.group_id]) {
                state.groups[obj.group_id] = obj
              } else {
                if (state.groups && state.groups[obj.group_id] && state.groups[obj.group_id]['last_message_id'] !== obj['last_message_id'] ) {
                  mergeObj(state.groups[obj.group_id], obj)
                }
                if (state.groups && state.groups[obj.group_id] && state.groups[obj.group_id] && obj['blocked_uins'] &&  state.groups[obj.group_id]['blocked_uins'] && state.groups[obj.group_id]['blocked_uins'].constructor === Array && obj['blocked_uins'].constructor === Array && state.groups[obj.group_id]['blocked_uins'].length !== obj['blocked_uins'].length ) {
                  mergeObj(state.groups[obj.group_id], obj)
                }
              }
            }
    ...
    case 'NETA_GROUPS_UPDATE_MESS_DRAFT': {
      if (typeof (state.groups) === 'undefined') { state.groups = {} }
      if(state.groups && state.groups[action.id]){
        state.groups[action.id].draft_mess = {
          'created_at': action.data.created_at,
          'message': action.data.message
        }
        state.groups[action.id].group_fullname = action.data.group_fullname || ''
...
export const netaGroupJoinCall = (state = initialState.netaGroupJoinCall, action) => {
  switch (action.type) {
    case 'NETA_GROUP_CALL_UPDATE': {
      if (typeof (state.groups) === 'undefined') { state.groups = {} }

      if (action && action.id && !state.groups[action.id.toString()] && action.data) {
        state.groups[action.id] = action.data

        const newState = { status: !state.status, groups: state.groups }
        
        rxsetLocal('netaGroupJoinCall', JSON.stringify(newState))
    ...
    case 'NETA_GROUP_CALL_STATUS_UPDATE': {
      if (typeof (state.groups) === 'undefined') { state.groups = {} }

      if (action && action.id && state.groups[action.id]) {
        state.groups[action.id]['status'] = (action && action.data) ? action.data.status : true
        state.groups[action.id]['oncall'] = (action && action.data) ? action.data.oncall : false
...
export const netaGroupsDraft = (state = initialState.netaGroupsDraft, action)=>{
  switch (action.type) {
    case 'UPDATE_GROUPS_DRAFT':{
      if (!state) { state = { status: false, groups: {} } }
      if (typeof (state.groups) === 'undefined') { state.groups = {} }
      ...
      const newState = { status: !state.status, groups: state.groups }
      rxsetLocal('netaGroupsDraft', JSON.stringify(newState))
      return newState
      break

    }

    case 'REMOVE_GROUPS_DRAFT':{
      if (typeof (state.groups) === 'undefined') { state.groups = {} }
...
export const netaMess = (state = initialStateLate.netaMess, action) => {
  switch (action.type) {
    ...
    case 'NETA_MESS_UPDATE': {
      if (action && action.mess && action.mess.group_id && action.mess.message_id) {
        if (state && state.data && state.data[action.mess.group_id] && state.data[action.mess.group_id]['messages'] && state.data[action.mess.group_id]['messages'][action.mess.created_at]) {
          state.data[action.mess.group_id]['messages'][action.mess.created_at] = action.mess
        }
      }
      rxsetLocal('netaMess', JSON.stringify(state))
      return state
    }
  ...
  switch (action.type) {
    case 'USER_STATUS_UPDATE': {
      ...
      if (action.data && action.data.uin) {
        netaUsersStatus[Number(action.data.uin)] = action.data
        if (netaUsersStatus && netaUsersStatus[Number(action.data.uin)] && netaUsersStatus[Number(action.data.uin)]['uin']) {
...
export const reducers = combineReducers({
  ...
  netaCall, usersStatus, netaPopup, userClickInfo, groupMess, netaGroupCall, netaGroupJoinCall, netaGroupsDraft
})
